...
async def create_task(db: AsyncSession, task_in: TaskCreate, user_id: int) -> Task:
    ...
    query = select(Task).options(joinedload(Task.category)).where(Task.id == db_task.id)
    result = await db.execute(query)
...
async def update_task(
    db: AsyncSession, task_id: int, task_in: TaskUpdate, user_id: int
) -> Task:
    """
...
async def get_tasks_for_user(
    ...
    if category_filter == "null":
        ...
        category = await get_category_if_owned(
            db=db, category_id=parsed_category_id, user_id=user_id
        )
...
async def delete_the_task(db: AsyncSession, task_id: int, user_id: int) -> None:
    """
    Delete task.
...
async def get_task_statistics_for_user(
    ...
    total = await db.scalar(select(func.count(Task.id)).where(Task.user_id == user_id))
