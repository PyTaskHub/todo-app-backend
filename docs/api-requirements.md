...
* Базовый префикс всех версионированных маршрутов: `/api/v1`
...
Изменение `username` и пароля выполняется через отдельные эндпоинты.
...
Если какое-то поле не передано, оно не изменяется.
...
`user_id` берётся из текущего пользователя и в теле запроса не передаётся.
...
### 4.8. GET /api/v1/tasks/statistics - статистика по задачам
...
Формат ответа определяется реализацией сервиса (см. фактический контракт в проекте/Swagger).
...
Категории принадлежат конкретному пользователю и уникальны по паре (`name`, `user_id`).
